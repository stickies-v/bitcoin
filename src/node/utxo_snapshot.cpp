...
bool WriteSnapshotBaseBlockhash(Chainstate& snapshot_chainstate)
{
    ...
    AutoFile afile{file};
    if (afile.IsNull()) {
        KernelLogInfo("[snapshot] failed to open base blockhash file for writing: %s\n",
    ...
    if (afile.fclose() != 0) {
        KernelLogInfo("[snapshot] failed to close base blockhash file %s after writing\n",
                      fs::PathToString(write_to));
        return false;
    }
    return true;
...
std::optional<uint256> ReadSnapshotBaseBlockhash(fs::path chaindir)
{
    if (!fs::exists(chaindir)) {
        KernelLogInfo("[snapshot] cannot read base blockhash: no chainstate dir "
                      "exists at path %s\n",
    ...
    if (!fs::exists(read_from)) {
        KernelLogInfo("[snapshot] snapshot chainstate dir is malformed! no base blockhash file "
    ...
    AutoFile afile{file};
    if (afile.IsNull()) {
        KernelLogInfo("[snapshot] failed to open base blockhash file for reading: %s\n",
                      read_from_str);
        return std::nullopt;
    }
    ...
    if (position != afile.tell()) {
        KernelLogWarning("[snapshot] warning: unexpected trailing data in %s\n", read_from_str);
    }
