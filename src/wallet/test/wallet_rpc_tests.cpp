...
std::string TestWalletName(const std::string& endpoint, std::optional<std::string> parameter = std::nullopt)
{
    JSONRPCRequest req;
    req.URI = endpoint;
    return wallet::EnsureUniqueWalletName(req, parameter ? &*parameter : nullptr);
}
