    ...
    const CTransaction& tx)
{
    ...
    for (size_t i = 0; i < tx.vin.size(); ++i) {
        if (auto coin{coins.GetCoin(tx.vin[i].prevout)}) {
            ...
            KernelLogInfo("ERROR: %s: Missing input %d in transaction \'%s\'\n", __func__, i, tx.GetHash().GetHex());
            return std::nullopt;
        }
    }
    return prev_heights;
    ...
    EXCLUSIVE_LOCKS_REQUIRED(::cs_main, pool.cs)
{
    ...
    if (expired != 0) {
        KernelLogDebug(kernel::Category::MEMPOOL, "Expired %i transactions from the memory pool\n", expired);
    }
                                         ...
                                         PackageValidationState& package_state)
{
    ...
    KernelLogDebug(kernel::Category::TXPACKAGES, "package RBF checks passed: parent %s (wtxid=%s), child %s (wtxid=%s), package hash (%s)\n",
...
bool MemPoolAccept::ConsensusScriptChecks(const ATMPArgs& args, Workspace& ws)
{
    ...
    if (!CheckInputsFromMempoolAndCache(tx, state, m_view, m_pool, currentBlockScriptVerifyFlags,
        ...
        KernelLogInfo("BUG! PLEASE REPORT THIS! CheckInputScripts failed against latest-block but not STANDARD flags %s, %s\n", hash.ToString(), state.ToString());
        return Assume(false);
    }
...
void MemPoolAccept::FinalizeSubpackage(const ATMPArgs& args)
{
    ...
    for (CTxMemPool::txiter it : m_subpackage.m_changeset->GetRemovals())
    {
        ...
        KernelLogDebug(kernel::Category::MEMPOOL, "%s\n", log_string);
                                  ...
                                  std::map<Wtxid, MempoolAcceptResult>& results)
{
    ...
    if (!m_subpackage.m_replaced_transactions.empty()) {
        KernelLogDebug(kernel::Category::MEMPOOL, "replaced %u mempool transactions with %u new one(s) for %s additional fees, %d delta bytes\n",
...
MempoolAcceptResult MemPoolAccept::AcceptSingleTransaction(const CTransactionRef& ptx, ATMPArgs& args)
{
    ...
    if (!m_subpackage.m_replaced_transactions.empty()) {
        KernelLogDebug(kernel::Category::MEMPOOL, "replaced %u mempool transactions with 1 new transaction for %s additional fees, %d delta bytes\n",
...
bool ChainstateManager::IsInitialBlockDownload() const
{
    ...
    KernelLogInfo("Leaving InitialBlockDownload (latching to false)");
...
void Chainstate::CheckForkWarningConditions()
{
    ...
    if (m_chainman.m_best_invalid && m_chainman.m_best_invalid->nChainWork > m_chain.Tip()->nChainWork + (GetBlockProof(*m_chain.Tip()) * 6)) {
        KernelLogInfo("%s: Warning: Found invalid chain at least ~6 blocks longer than our best chain.\nChain state database corruption likely.\n", __func__);
...
void Chainstate::InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    KernelLogInfo("%s: invalid block=%s  height=%d  log2_work=%f  date=%s\n", __func__,
                  pindexNew->GetBlockHash().ToString(), pindexNew->nHeight,
                  log(pindexNew->nChainWork.getdouble()) / log(2.0), FormatISO8601DateTime(pindexNew->GetBlockTime()));
    CBlockIndex *tip = m_chain.Tip();
    assert (tip);
    KernelLogInfo("%s:  current best=%s  height=%d  log2_work=%f  date=%s\n", __func__,
    ...
    : m_signature_cache{signature_cache_bytes}
{
    ...
    KernelLogInfo("Using %zu MiB out of %zu MiB requested for script execution cache, able to store %zu elements",
...
DisconnectResult Chainstate::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view)
{
    ...
    if (!m_blockman.ReadBlockUndo(blockUndo, *pindex)) {
        KernelLogError("DisconnectBlock(): failure reading undo data\n");
        return DISCONNECT_FAILED;
    }

    if (blockUndo.vtxundo.size() + 1 != block.vtx.size()) {
        KernelLogError("DisconnectBlock(): block and undo data inconsistent\n");
        return DISCONNECT_FAILED;
    }
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
        ...
        if (i > 0) { // not coinbases
            ...
            if (txundo.vprevout.size() != tx.vin.size()) {
                KernelLogError("DisconnectBlock(): transaction and undo data inconsistent\n");
                return DISCONNECT_FAILED;
            }
                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    if (!CheckBlock(block, state, params.GetConsensus(), !fJustCheck, !fJustCheck)) {
        ...
        KernelLogError("%s: Consensus::CheckBlock: %s\n", __func__, state.ToString());
        return false;
    }
    ...
    KernelLogDebug(kernel::Category::BENCH, "    - Sanity checks: %.2fms [%.2fs (%.2fms/blk)]\n",
    ...
    KernelLogDebug(kernel::Category::BENCH, "    - Fork checks: %.2fms [%.2fs (%.2fms/blk)]\n",
    ...
    if (script_check_reason != m_last_script_check_reason_logged && GetRole() == ChainstateRole::NORMAL) {
        if (fScriptChecks) {
            KernelLogInfo("Enabling script verification at block #%d (%s): %s.",
                          pindex->nHeight, block_hash.ToString(), script_check_reason);
        } else {
            KernelLogInfo("Disabling script verification at block #%d (%s).",
    ...
    KernelLogDebug(kernel::Category::BENCH, "      - Connect %u transactions: %.2fms (%.3fms/tx, %.3fms/txin) [%.2fs (%.2fms/blk)]\n", (unsigned)block.vtx.size(),
    ...
    if (!state.IsValid()) {
        KernelLogInfo("Block validation error: %s", state.ToString());
        return false;
    }
    const auto time_4{SteadyClock::now()};
    m_chainman.time_verify += time_4 - time_2;
    KernelLogDebug(kernel::Category::BENCH, "    - Verify %u txins: %.2fms (%.3fms/txin) [%.2fs (%.2fms/blk)]\n", nInputs - 1,
    ...
    KernelLogDebug(kernel::Category::BENCH, "    - Write undo data: %.2fms [%.2fs (%.2fms/blk)]\n",
    ...
    KernelLogDebug(kernel::Category::BENCH, "    - Index writing: %.2fms [%.2fs (%.2fms/blk)]\n",
    ...
    size_t max_mempool_size_bytes)
{
    ...
    if (cacheSize > nTotalSpace) {
        KernelLogInfo("Cache size (%s) exceeds total space (%s)\n", cacheSize, nTotalSpace);
    ...
    int nManualPruneHeight)
{
    ...
    try {
    {
        ...
        if (m_blockman.IsPruneMode() && (m_blockman.m_check_for_pruning || nManualPruneHeight > 0) && m_chainman.m_blockman.m_blockfiles_indexed) {
            ...
            if (limiting_lock) {
                KernelLogDebug(kernel::Category::PRUNE, "%s limited pruning to height %d\n", limiting_lock.value(), last_prune);
            }
        ...
        if (should_write) {
            KernelLogDebug(kernel::Category::COINDB, "Writing chainstate to disk: flush mode=%s, prune=%d, large=%d, critical=%d, periodic=%d",
            ...
            }
            {
                ...
                if (!m_blockman.FlushChainstateBlockFile(m_chain.Height())) {
                    KernelLogWarning("%s: Failed to flush block file.", __func__);
                }
            }
...
void Chainstate::ForceFlushStateToDisk()
{
    ...
    if (!this->FlushStateToDisk(state, FlushStateMode::ALWAYS)) {
        KernelLogInfo("%s: failed to flush state (%s)\n", __func__, state.ToString());
    }
}
    ...
    const std::string& warning_messages) EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    KernelLogInfo("%s%s: new best=%s height=%d version=0x%08x log2_work=%f tx=%lu date='%s' progress=%f cache=%.1fMiB(%utxo)%s\n",
                  prefix, func_name,
...
bool Chainstate::DisconnectTip(BlockValidationState& state, DisconnectedBlockTransactions* disconnectpool)
{
    ...
    if (!m_blockman.ReadBlock(block, *pindexDelete)) {
        KernelLogError("DisconnectTip(): Failed to read block\n");
        return false;
    }
    ...
    const auto time_start{SteadyClock::now()};
    {
        ...
        if (DisconnectBlock(block, pindexDelete, view) != DISCONNECT_OK) {
            KernelLogError("DisconnectTip(): DisconnectBlock %s failed\n", pindexDelete->GetBlockHash().ToString());
            return false;
        }
        bool flushed = view.Flush();
        assert(flushed);
    }
    KernelLogDebug(kernel::Category::BENCH, "- Disconnect block: %.2fms\n",
...

    {
        ...
        for (auto& prune_lock : m_blockman.m_prune_locks) {
            ...
            KernelLogDebug(kernel::Category::PRUNE, "%s prune lock moved back to %d\n", prune_lock.first, max_height_first);
        }
    }
    ...
    DisconnectedBlockTransactions& disconnectpool)
{
    ...
    if (!block_to_connect) {
        ...
        KernelLogDebug(kernel::Category::BENCH, "  - Using cached block\n");
    }
    // Apply the block atomically to the chain state.
    const auto time_2{SteadyClock::now()};
    ...
    KernelLogDebug(kernel::Category::BENCH, "  - Load block from disk: %.2fms\n",
                   Ticks<MillisecondsDouble>(time_2 - time_1));
    {
        CCoinsViewCache view(&CoinsTip());
        ...
        if (!rv) {
            ...
            KernelLogError("%s: ConnectBlock %s failed, %s\n", __func__, pindexNew->GetBlockHash().ToString(), state.ToString());
            return false;
        }
        time_3 = SteadyClock::now();
        m_chainman.time_connect_total += time_3 - time_2;
        assert(m_chainman.num_blocks_total > 0);
        KernelLogDebug(kernel::Category::BENCH, "  - Connect total: %.2fms [%.2fs (%.2fms/blk)]\n",
    ...
    KernelLogDebug(kernel::Category::BENCH, "  - Flush: %.2fms [%.2fs (%.2fms/blk)]\n",
    ...
    KernelLogDebug(kernel::Category::BENCH, "  - Writing chainstate: %.2fms [%.2fs (%.2fms/blk)]\n",
                   Ticks<MillisecondsDouble>(time_5 - time_4),
    ...
    KernelLogDebug(kernel::Category::BENCH, "  - Connect postprocess: %.2fms [%.2fs (%.2fms/blk)]\n",
                   Ticks<MillisecondsDouble>(time_6 - time_5),
    ...
    KernelLogDebug(kernel::Category::BENCH, "- Connect block: %.2fms [%.2fs (%.2fms/blk)]\n",
...
bool Chainstate::ActivateBestChain(BlockValidationState& state, std::shared_ptr<const CBlock> pblock)
{
    ...
    if (WITH_LOCK(::cs_main, return m_disabled)) {
        KernelLogInfo("m_disabled is set - this chainstate should not be in operation. "
...
void ChainstateManager::ReceivedBlockTransactions(const CBlock& block, CBlockIndex* pindexNew, const FlatFilePos& pos)
{
    ...
    if (!Assume(pindexNew->m_chain_tx_count == 0 || pindexNew->m_chain_tx_count == prev_tx_sum(*pindexNew) ||
        ...
        KernelLogWarning("Internal bug detected: block %d has unexpected m_chain_tx_count %i that should be %i (%s %s). Please report this issue here: %s\n",
...
bool IsBlockMutated(const CBlock& block, bool check_witness_root)
{
    ...
    if (!CheckMerkleRoot(block, state)) {
        KernelLogDebug(kernel::Category::VALIDATION, "Block mutated: %s\n", state.ToString());
        return true;
    }
    ...
    if (hash != GetConsensus().hashGenesisBlock) {
        if (miSelf != m_blockman.m_block_index.end()) {
            ...
            if (pindex->nStatus & BLOCK_FAILED_MASK) {
                KernelLogDebug(kernel::Category::VALIDATION, "%s: block %s is marked invalid\n", __func__, hash.ToString());
        ...
        if (!CheckBlockHeader(block, state, GetConsensus())) {
            KernelLogDebug(kernel::Category::VALIDATION, "%s: Consensus::CheckBlockHeader: %s, %s\n", __func__, hash.ToString(), state.ToString());
            return false;
        }
        ...
        if (mi == m_blockman.m_block_index.end()) {
            KernelLogDebug(kernel::Category::VALIDATION, "header %s has prev block not found: %s\n", hash.ToString(), block.hashPrevBlock.ToString());
            return state.Invalid(BlockValidationResult::BLOCK_MISSING_PREV, "prev-blk-not-found");
        }
        pindexPrev = &((*mi).second);
        if (pindexPrev->nStatus & BLOCK_FAILED_MASK) {
            KernelLogDebug(kernel::Category::VALIDATION, "header %s has prev block invalid: %s\n", hash.ToString(), block.hashPrevBlock.ToString());
            return state.Invalid(BlockValidationResult::BLOCK_INVALID_PREV, "bad-prevblk");
        }
        if (!ContextualCheckBlockHeader(block, state, m_blockman, *this, pindexPrev)) {
            KernelLogDebug(kernel::Category::VALIDATION, "%s: Consensus::ContextualCheckBlockHeader: %s, %s\n", __func__, hash.ToString(), state.ToString());
            return false;
        }
    }
    if (!min_pow_checked) {
        KernelLogDebug(kernel::Category::VALIDATION, "%s: not adding new block header %s, missing anti-dos proof-of-work validation\n", __func__, hash.ToString());
...
bool ChainstateManager::ProcessNewBlockHeaders(std::span<const CBlockHeader> headers, bool min_pow_checked, BlockValidationState& state, const CBlockIndex** ppindex)
{
    ...
    if (NotifyHeaderTip()) {
        if (IsInitialBlockDownload() && ppindex && *ppindex) {
            ...
            KernelLogInfo("Synchronizing blockheaders, height: %d (~%.2f%%)\n", last_accepted.nHeight, progress);
        }
    }
    return true;
}
...
void ChainstateManager::ReportHeadersPresync(const arith_uint256& work, int64_t height, int64_t timestamp)
{
    ...
    if (initial_download) {
        ...
        KernelLogInfo("Pre-synchronizing blockheaders, height: %d (~%.2f%%)\n", height, progress);
    }
}
...
bool ChainstateManager::ProcessNewBlock(const std::shared_ptr<const CBlock>& block, bool force_processing, bool min_pow_checked, bool* new_block)
{
...

    {
        ...
        if (!ret) {
            ...
            KernelLogError("%s: AcceptBlock FAILED (%s)\n", __func__, state.ToString());
            return false;
        }
    }
    ...
    if (!ActiveChainstate().ActivateBestChain(state, block)) {
        KernelLogError("%s: ActivateBestChain failed (%s)\n", __func__, state.ToString());
        return false;
    }
    ...
    if (bg_chain && !bg_chain->ActivateBestChain(bg_state, block)) {
        KernelLogError("%s: [background] ActivateBestChain failed (%s)\n", __func__, bg_state.ToString());
        return false;
     }
...
bool Chainstate::LoadChainTip()
{
    ...
    tip = m_chain.Tip();
    KernelLogInfo("Loaded best chain: hashBestChain=%s height=%d date=%s progress=%f",
    ...
    int nCheckLevel, int nCheckDepth)
{
    ...
    KernelLogInfo("Verifying last %i blocks at level %i", nCheckDepth, nCheckLevel);
    ...
    KernelLogInfo("Verification progress: 0%%");
    ...
    for (pindex = chainstate.m_chain.Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
        ...
        if (reportDone < percentageDone / 10) {
            ...
            KernelLogInfo("Verification progress: %d%%", percentageDone);
        ...
        if ((chainstate.m_blockman.IsPruneMode() || is_snapshot_cs) && !(pindex->nStatus & BLOCK_HAVE_DATA)) {
            ...
            KernelLogInfo("Block verification stopping at height %d (no data). This could be due to pruning or use of an assumeutxo snapshot.", pindex->nHeight);
        ...
        if (!chainstate.m_blockman.ReadBlock(block, *pindex)) {
            KernelLogInfo("Verification error: ReadBlock failed at %d, hash=%s\n", pindex->nHeight, pindex->GetBlockHash().ToString());
            return VerifyDBResult::CORRUPTED_BLOCK_DB;
        }
        // check level 1: verify block validity
        if (nCheckLevel >= 1 && !CheckBlock(block, state, consensus_params)) {
            KernelLogInfo("Verification error: found bad block at %d, hash=%s (%s)\n",
        ...
        if (nCheckLevel >= 2 && pindex) {
            ...
            if (!pindex->GetUndoPos().IsNull()) {
                if (!chainstate.m_blockman.ReadBlockUndo(undo, *pindex)) {
                    KernelLogInfo("Verification error: found bad undo data at %d, hash=%s\n", pindex->nHeight, pindex->GetBlockHash().ToString());
                    return VerifyDBResult::CORRUPTED_BLOCK_DB;
                }
            }
        }
        ...
        if (nCheckLevel >= 3) {
            if (curr_coins_usage <= chainstate.m_coinstip_cache_size_bytes) {
                ...
                if (res == DISCONNECT_FAILED) {
                    KernelLogInfo("Verification error: irrecoverable inconsistency in block data at %d, hash=%s\n", pindex->nHeight, pindex->GetBlockHash().ToString());
                    return VerifyDBResult::CORRUPTED_BLOCK_DB;
                }
    ...
    if (pindexFailure) {
        KernelLogInfo("Verification error: coin database inconsistencies found (last %i blocks, %i good transactions before that)\n", chainstate.m_chain.Height() - pindexFailure->nHeight + 1, nGoodTransactions);
        return VerifyDBResult::CORRUPTED_BLOCK_DB;
    }
    if (skipped_l3_checks) {
        KernelLogInfo("Skipped verification of level >=3 (insufficient database cache size). Consider increasing -dbcache.\n");
    }
    ...
    if (nCheckLevel >= 4 && !skipped_l3_checks) {
        while (pindex != chainstate.m_chain.Tip()) {
            ...
            if (!chainstate.ConnectBlock(block, state, pindex, coins)) {
                KernelLogInfo("Verification error: found unconnectable block at %d, hash=%s (%s)\n", pindex->nHeight, pindex->GetBlockHash().ToString(), state.ToString());
                return VerifyDBResult::CORRUPTED_BLOCK_DB;
            }
            if (chainstate.m_chainman.m_interrupt) return VerifyDBResult::INTERRUPTED;
        }
    ...
    KernelLogInfo("Verification: No coin database inconsistencies in last %i blocks (%i transactions)", block_count, nGoodTransactions);
...
bool Chainstate::RollforwardBlock(const CBlockIndex* pindex, CCoinsViewCache& inputs)
{
    ...
    if (!m_blockman.ReadBlock(block, *pindex)) {
        KernelLogError("ReplayBlock(): ReadBlock failed at %d, hash=%s\n", pindex->nHeight, pindex->GetBlockHash().ToString());
        return false;
    }
...
bool Chainstate::ReplayBlocks()
{
    ...
    if (hashHeads.size() != 2) {
        KernelLogError("ReplayBlocks(): unknown inconsistent state\n");
        return false;
    }

    m_chainman.GetNotifications().progress(_("Replaying blocks…"), 0, false);
    KernelLogInfo("Replaying blocks");
    ...
    if (m_blockman.m_block_index.count(hashHeads[0]) == 0) {
        KernelLogError("ReplayBlocks(): reorganization to unknown block requested\n");
        return false;
    }
    ...
    if (!hashHeads[1].IsNull()) { // The old tip is allowed to be 0, indicating it's the first flush.
        if (m_blockman.m_block_index.count(hashHeads[1]) == 0) {
            KernelLogError("ReplayBlocks(): reorganization from unknown block requested\n");
            return false;
        }
    ...
    while (pindexOld != pindexFork) {
        if (pindexOld->nHeight > 0) { // Never disconnect the genesis block.
            ...
            if (!m_blockman.ReadBlock(block, *pindexOld)) {
                KernelLogError("RollbackBlock(): ReadBlock() failed at %d, hash=%s\n", pindexOld->nHeight, pindexOld->GetBlockHash().ToString());
                return false;
            }
            KernelLogInfo("Rolling back %s (%i)", pindexOld->GetBlockHash().ToString(), pindexOld->nHeight);
            DisconnectResult res = DisconnectBlock(block, pindexOld, cache);
            if (res == DISCONNECT_FAILED) {
                KernelLogError("RollbackBlock(): DisconnectBlock failed at %d, hash=%s\n", pindexOld->nHeight, pindexOld->GetBlockHash().ToString());
                return false;
            }
    ...
    for (int nHeight = nForkHeight + 1; nHeight <= pindexNew->nHeight; ++nHeight) {
        ...
        KernelLogInfo("Rolling forward %s (%i)", pindex.GetBlockHash().ToString(), nHeight);
...
bool Chainstate::LoadGenesisBlock()
{
    ...
    try {
        ...
        if (blockPos.IsNull()) {
            KernelLogError("%s: writing genesis block to disk failed\n", __func__);
            return false;
        }
        CBlockIndex* pindex = m_blockman.AddToBlockIndex(block, m_chainman.m_best_header);
        m_chainman.ReceivedBlockTransactions(block, pindex, blockPos);
    } catch (const std::runtime_error& e) {
        KernelLogError("%s: failed to write genesis block: %s\n", __func__, e.what());
        return false;
    }
            ...
            try {
...

                {
                    ...
                    if (hash != params.GetConsensus().hashGenesisBlock && !m_blockman.LookupBlockIndex(header.hashPrevBlock)) {
                        KernelLogDebug(kernel::Category::REINDEX, "%s: Out of order block %s, parent %s not known\n", __func__, hash.ToString(),
                    ...
                    if (!pindex || (pindex->nStatus & BLOCK_HAVE_DATA) == 0) {
                        ...
                        KernelLogDebug(kernel::Category::REINDEX, "Block Import: already had block %s at height %d\n", hash.ToString(), pindex->nHeight);
                    }
                }
                ...
                if (m_blockman.IsPruneMode() && m_blockman.m_blockfiles_indexed && pblock) {
                    ...
                    for (auto c : GetAll()) {
                        ...
                        if (!c->ActivateBestChain(state, pblock)) {
                            KernelLogDebug(kernel::Category::REINDEX, "failed to activate chain (%s)\n", state.ToString());
                ...
                while (!queue.empty()) {
                    ...
                    while (range.first != range.second) {
                        ...
                        if (m_blockman.ReadBlock(*pblockrecursive, it->second, {})) {
                            ...
                            KernelLogDebug(kernel::Category::REINDEX, "%s: Processing out of order child %s of %s", __func__, block_hash.ToString(), head.ToString());
                            LOCK(cs_main);
                ...
                KernelLogDebug(kernel::Category::REINDEX, "%s: unexpected data at file offset 0x%x - %s. continuing\n", __func__, (nRewind - 1), e.what());
            }
        }
    } catch (const std::runtime_error& e) {
        GetNotifications().fatalError(strprintf(_("System error while loading external block file: %s"), e.what()));
    }
    KernelLogInfo("Loaded %i blocks from external file in %dms", nLoaded, Ticks<std::chrono::milliseconds>(SteadyClock::now() - start));
}
...
bool Chainstate::ResizeCoinsCaches(size_t coinstip_size, size_t coinsdb_size)
{
    ...
    KernelLogInfo("[%s] resized coinsdb cache to %.1f MiB",
                  this->ToString(), coinsdb_size * (1.0 / 1024 / 1024));
    KernelLogInfo("[%s] resized coinstip cache to %.1f MiB",
...
double ChainstateManager::GuessVerificationProgress(const CBlockIndex* pindex) const
{
    ...
    if (pindex->m_chain_tx_count == 0) {
        KernelLogDebug(kernel::Category::VALIDATION, "Block %d has unset m_chain_tx_count. Unable to estimate verification progress.\n", pindex->nHeight);
        return 0.0;
    }
    ...
    EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    if (is_snapshot) {
        ...
        try {
            ...
            if (!existed) {
                KernelLogInfo("[snapshot] snapshot chainstate dir being removed lacks %s file\n",
                              fs::PathToString(node::SNAPSHOT_BLOCKHASH_FILENAME));
            }
        } catch (const fs::filesystem_error& e) {
            KernelLogWarning("[snapshot] failed to remove file %s: %s\n",
                             fs::PathToString(base_blockhash_path), e.code().message());
        }
    }

    std::string path_str = fs::PathToString(db_path);
    KernelLogInfo("Removing leveldb dir at %s\n", path_str);
    ...
    if (!destroyed) {
        KernelLogInfo("error: leveldb DestroyDB call failed on %s\n", path_str);
    }
        ...
        bool in_memory)
{
    ...
    KernelLogInfo("[snapshot] successfully activated snapshot %s", base_blockhash.ToString());
    KernelLogInfo("[snapshot] (%.2f MB)",
    ...
    const SnapshotMetadata& metadata)
{
    ...
    KernelLogInfo("[snapshot] loading %d coins from snapshot %s", coins_left, base_blockhash.ToString());
        ...
        try {
            ...
            for (size_t i = 0; i < coins_per_txid; i++) {
                ...
                if (coins_processed % 1000000 == 0) {
                    KernelLogInfo("[snapshot] %d coins loaded (%.2f%%, %.2f MB)",
                                  coins_processed,
    ...
    KernelLogInfo("[snapshot] loaded %d (%.2f MB) coins from snapshot %s",
                  coins_count,
                  coins_cache.DynamicMemoryUsage() / (1000 * 1000),
                  base_blockhash.ToString());
    ...
    KernelLogInfo("[snapshot] validated snapshot (%.2f MB)",
                  coins_cache.DynamicMemoryUsage() / (1000 * 1000));
    return {};
}
...
SnapshotCompletionResult ChainstateManager::MaybeCompleteSnapshotValidation()
{
    ...
    auto handle_invalid_snapshot = [&]() EXCLUSIVE_LOCKS_REQUIRED(::cs_main) {
        ...
        KernelLogError("[snapshot] !!! %s\n", user_error.original);
        KernelLogError("[snapshot] deleting snapshot, reverting to validated chain, and stopping node\n");
    ...
    if (index_new.GetBlockHash() != snapshot_blockhash) {
        KernelLogInfo("[snapshot] supposed base block %s does not match the "
    ...
    if (!maybe_au_data) {
        KernelLogInfo("[snapshot] assumeutxo data not found for height "
    ...
    KernelLogInfo("[snapshot] computing UTXO stats for background chainstate to validate "
    ...
    if (!maybe_ibd_stats) {
        KernelLogInfo("[snapshot] failed to generate stats for validation coins db\n");
    ...
    if (AssumeutxoHash{ibd_stats.hashSerialized} != au_data.hash_serialized) {
        KernelLogInfo("[snapshot] hash mismatch: actual=%s, expected=%s\n",
    ...
    KernelLogInfo("[snapshot] snapshot beginning at %s has been fully validated",
...
void ChainstateManager::MaybeRebalanceCaches()
{
    ...
    if (ibd_usable && !snapshot_usable) {
        ...
        KernelLogInfo("[snapshot] allocating all cache to the snapshot chainstate");
...
bool ChainstateManager::DetectSnapshotChainstate()
{
    ...
    KernelLogInfo("[snapshot] detected active snapshot chainstate (%s) - loading",
...
Chainstate& ChainstateManager::ActivateExistingSnapshot(uint256 base_blockhash)
{
    ...
    KernelLogInfo("[snapshot] switching active chainstate to %s", m_snapshot_chainstate->ToString());
...
util::Result<void> Chainstate::InvalidateCoinsDBOnDisk()
{
    ...
    KernelLogInfo("[snapshot] renaming snapshot datadir %s to %s", dbpath, target);
    ...
    try {
        ...
        KernelLogInfo("%s: error renaming file '%s' -> '%s': %s\n",
...
bool ChainstateManager::DeleteSnapshotChainstate()
{
    ...
    if (!DeleteCoinsDBFromDisk(snapshot_datadir, /*is_snapshot=*/ true)) {
        KernelLogInfo("Deletion of %s failed. Please remove it manually to continue reindexing.\n",
                      fs::PathToString(snapshot_datadir));
        return false;
    }
    m_active_chainstate = m_ibd_chainstate.get();
...
bool ChainstateManager::ValidatedSnapshotCleanup()
{
    ...
    if (!ibd_chainstate_path_maybe || !snapshot_chainstate_path_maybe) {
        KernelLogInfo("[snapshot] snapshot chainstate cleanup cannot happen with "
    ...
    KernelLogInfo("[snapshot] deleting background chainstate directory (now unnecessary) (%s)",
    ...
    auto rename_failed_abort = [this](
        ...
        KernelLogError("[snapshot] Error renaming path (%s) -> (%s): %s\n",
    ...
    KernelLogInfo("[snapshot] moving snapshot chainstate (%s) to "
    ...
    if (!DeleteCoinsDBFromDisk(tmp_old, /*is_snapshot=*/false)) {
        ...
        KernelLogInfo("Deletion of %s failed. Please remove it manually, as the "
                      "directory is now unnecessary.\n",
                      fs::PathToString(tmp_old));
    } else {
        KernelLogInfo("[snapshot] deleted background chainstate directory (%s)",
                      fs::PathToString(ibd_chainstate_path));
    }
    return true;
}
