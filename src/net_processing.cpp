...
public:
    ...
    void BlockChecked(std::shared_ptr<const CBlock> block, const BlockValidationState& state) override
        EXCLUSIVE_LOCKS_REQUIRED(!m_peer_mutex);
 ...
 */
void PeerManagerImpl::BlockChecked(std::shared_ptr<const CBlock> block, const BlockValidationState& state)
{
    LOCK(cs_main);
